...
def load_json_manifest(manifest_path: str, required_keys: Optional[List[str]] = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        raise PermissionError(f"Cannot read manifest file (permission denied): {manifest_path}") from e
    except json.JSONDecodeError as e:
        raise ManifestError(f"Invalid JSON in manifest file '{manifest_path}': {e}") from e
    except Exception as e:
        raise ManifestError(f"Failed to load manifest file '{manifest_path}': {e}") from e

    # Validate required keys if specified
    if required_keys:
        missing_keys = [key for key in required_keys if key not in manifest_data]
        if missing_keys:
            raise ManifestError(f"Manifest '{manifest_path}' missing required keys: {missing_keys}")
...
def save_json_manifest(manifest_data: Dict[str, Any], output_path: str) -> None:
    ...
    try:
        ...
        raise PermissionError(f"Cannot write to manifest file (permission denied): {output_path}") from e
    except Exception as e:
        raise OSError(f"Failed to save manifest file '{output_path}': {e}") from e


def extract_genomes_from_manifest(manifest_data: Dict[str, Any]) -> List[Dict[str, Any]]:
    """
    ...
    elif "analysis_results" in manifest_data and "genomes" in manifest_data["analysis_results"]:
