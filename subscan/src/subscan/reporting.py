...
class ReportGenerator:
            ...
            if 'input_manifest' in self.manifest_data:
                ...
                if isinstance(input_manifest, dict):
                    ...
                    if 'metadata_csv' in input_manifest:
                        self.aggregated_data['file_paths']['metadata_csv'] = input_manifest['metadata_csv']
                    elif 'output_files' in input_manifest and 'metadata_csv' in input_manifest['output_files']:
                        self.aggregated_data['file_paths']['metadata_csv'] = input_manifest['output_files']['metadata_csv']
            
            # Look for mutation report path (from Domino 5)
            if 'mutation_report_path' in self.manifest_data:
                self.aggregated_data['file_paths']['mutation_report'] = self.manifest_data['mutation_report_path']
            elif 'output_files' in self.manifest_data and 'mutation_report' in self.manifest_data['output_files']:
                self.aggregated_data['file_paths']['mutation_report'] = self.manifest_data['output_files']['mutation_report']
            
            # Look for co-occurrence report path (from Domino 6)
            if 'cooccurrence_report_path' in self.manifest_data:
                self.aggregated_data['file_paths']['cooccurrence_report'] = self.manifest_data['cooccurrence_report_path']
            elif 'output_files' in self.manifest_data and 'cooccurrence_report' in self.manifest_data['output_files']:
                self.aggregated_data['file_paths']['cooccurrence_report'] = self.manifest_data['output_files']['cooccurrence_report']
                
            # Look for alignment manifest (needed for MSA viewer in Phase 3)
            if 'alignment_manifest_path' in self.manifest_data:
                self.aggregated_data['file_paths']['alignment_manifest'] = self.manifest_data['alignment_manifest_path']
            
            self.logger.info(f"Extracted file paths: {self.aggregated_data['file_paths']}")
    ...
    def _process_metadata_csv(self):
        ...
        try:
            ...
            self.aggregated_data['genome_metadata'] = {'genomes': [], 'count': 0, 'error': str(e)}
    ...
    def _process_mutation_report(self):
        ...
        try:
            ...
            self.aggregated_data['mutation_data'] = {
                ...
                'gene_families': mutation_df['gene_family'].unique().tolist() if 'gene_family' in mutation_df.columns else []
            }
            ...
            self.aggregated_data['mutation_data'] = {'mutations': [], 'count': 0, 'error': str(e)}
    ...
    def _process_cooccurrence_report(self):
        ...
        try:
            ...
            self.aggregated_data['cooccurrence_data'] = {'pairs': [], 'count': 0, 'error': str(e)}
    ...
    def _calculate_summary_statistics(self):
        ...
        stats = {
            ...
            'total_cooccurrence_pairs': self.aggregated_data['cooccurrence_data']['count'],
            'gene_families': len(self.aggregated_data['mutation_data'].get('gene_families', [])),
        ...
        if self.aggregated_data['mutation_data']['mutations']:
            ...
            for mutation in self.aggregated_data['mutation_data']['mutations']:
                ...
                gene_family_counts[gene_family] = gene_family_counts.get(gene_family, 0) + 1
    ...
    def _prepare_gene_family_distribution(self) -> Dict[str, Any]:
        ...
        if not self.aggregated_data or 'statistics' not in self.aggregated_data:
            return {
                'data': [],
                'layout': {
                    'title': 'No Gene Family Data Available'
                }
            }
            
        gene_family_dist = self.aggregated_data['statistics'].get('gene_family_distribution', {})
        ...
        sorted_families = sorted(gene_family_dist.items(), key=lambda x: x[1], reverse=True)
    ...
    def _prepare_cooccurrence_network(self) -> Dict[str, Any]:
        ...
        if not self.aggregated_data or 'cooccurrence_data' not in self.aggregated_data:
            return {
                ...
                'edges': [],
                'layout': {
                    'title': 'No Co-occurrence Data Available'
                }
            }
    ...
    def _prepare_genome_quality_scatter(self) -> Dict[str, Any]:
        ...
        if not self.aggregated_data or 'genome_metadata' not in self.aggregated_data:
            return {
                'data': [],
                'layout': {
                    'title': 'No Genome Quality Data Available'
                }
            }
        ...
        if not x_values:
            return {
                'data': [],
                'layout': {
                    'title': 'Insufficient Quality Metrics for Visualization'
                }
            }
    ...
    def prepare_msa_data(self) -> Dict[str, Any]:
        ...
        alignment_manifest_path = self.aggregated_data['file_paths'].get('alignment_manifest')
        
        if alignment_manifest_path and os.path.isfile(alignment_manifest_path):
            msa_data['alignments'] = self._process_alignment_files(alignment_manifest_path)
        else:
            self.logger.warning("No alignment manifest found - MSA viewer will be disabled")
            msa_data['alignments'] = []
        
        self.logger.info(f"Prepared {len(msa_data['alignments'])} alignments for MSAViewer")
        return msa_data
    
    def _process_alignment_files(self, alignment_manifest_path: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for gene_family, alignment_info in alignment_manifest.get('alignments', {}).items():
                alignment_file = alignment_info.get('alignment_file')
                
                if alignment_file and os.path.isfile(alignment_file):
                    alignment_data = self._read_alignment_file(alignment_file, gene_family)
                    if alignment_data:
    ...
    def _read_alignment_file(self, alignment_file: str, gene_family: str) -> Optional[Dict[str, Any]]:
    ...
    def generate_html_report(self, output_path: str, template_path: Optional[str] = None) -> str:
        """
    ...
    def _prepare_template_context(self, plotly_data: Dict[str, Any], msa_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        context = {
            ...
            'mutation_distribution_chart': json.dumps(plotly_data['mutation_distribution_chart']),
            'gene_family_bar_chart': json.dumps(plotly_data['gene_family_bar_chart']),
            'cooccurrence_network_data': json.dumps(plotly_data['cooccurrence_network_data']),
            'genome_quality_scatter': json.dumps(plotly_data['genome_quality_scatter']),
            'pipeline_completion_gauge': json.dumps(plotly_data['pipeline_completion_gauge']),
            ...
            'mutation_data': self.aggregated_data['mutation_data']['mutations'][:100],  # Limit for performance
            'cooccurrence_data': self.aggregated_data['cooccurrence_data']['pairs'][:100],  # Limit for performance
            ...
            'source_files': {
                ...
                'mutation_report': self.aggregated_data['file_paths'].get('mutation_report'),
                'cooccurrence_report': self.aggregated_data['file_paths'].get('cooccurrence_report')
            },
            ...
            'has_cooccurrence_data': self.aggregated_data['cooccurrence_data']['count'] > 0
        }
    ...
    def _render_template(self, template_content: str, context: Dict[str, Any]) -> str:
        ...
        try:
            ...
            env.filters['thousands'] = lambda x: f"{x:,}" if isinstance(x, (int, float)) else x
            env.filters['percentage'] = lambda x: f"{x:.1f}%" if isinstance(x, (int, float)) else x
            env.filters['truncate_table'] = lambda lst, n=10: lst[:n] if isinstance(lst, list) else []
            ...
            raise RuntimeError("Jinja2 is required for HTML report generation. Install with: pip install jinja2")
        except Exception as e:
    ...
    def generate_report_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'visualization_components': {
                ...
                'msa_alignments': len(self.aggregated_data.get('file_paths', {}).get('alignment_manifest', [])),
                'data_tables': 3  # Genome, mutation, co-occurrence tables
            },
            'file_sources': {
                'metadata_available': bool(self.aggregated_data['file_paths'].get('metadata_csv')),
                'mutations_available': bool(self.aggregated_data['file_paths'].get('mutation_report')),
                'cooccurrence_available': bool(self.aggregated_data['file_paths'].get('cooccurrence_report'))
            }
        }