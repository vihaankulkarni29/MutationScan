    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_genome_dir:
            ...
            command = abricate_command_base + [
                ...
                genome_output_dir,
                "--threads",
                "1",  # Single thread per worker to avoid conflicts
            ]
...
def find_abricate_results(output_dir, genomes):
    ...
    for genome in genomes:
        ...
        if accession:
            ...
            matching_files = [
                f for f in found_files if os.path.basename(f) == card_pattern
            ]
...
def main():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            success = execute_abricate_automator(
                temp_dir, args.output_dir, args.threads
            )
        ...
        manifest_path = generate_annotation_manifest(
            manifest, result_mapping, args.output_dir
        )
