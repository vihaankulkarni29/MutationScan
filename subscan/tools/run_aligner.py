...
def validate_arguments(args):
    ...
    if args.user_reference_dir:
        if not os.path.isdir(args.user_reference_dir):
            print(f"❌ Error: User reference directory not found: {args.user_reference_dir}")
            sys.exit(1)
        ...
        if not fasta_files:
            print(f"❌ Error: No FASTA files found in reference directory: {args.user_reference_dir}")
    ...
    if args.threads > max_threads:
        print(f"⚠️  Warning: Thread count ({args.threads}) exceeds recommended maximum ({max_threads})")
    ...
    if args.user_reference_dir:
        ...
        ref_count = len(list(Path(args.user_reference_dir).glob("*.fasta")) + 
...
def align_single_gene_family(work_item):
    ...
    try:
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(mode='w', suffix=f'_{gene_name}_combined.fasta', 
                                           delete=False, dir=output_dir) as temp_file:
                temp_combined_fasta = temp_file.name
                ...
                for protein_path in protein_paths:
                    ...
                    try:
                        ...
                        print(f"⚠️  Warning: Could not read protein file {protein_path}: {str(e)}")
                        continue
                
                if protein_count == 0:
                    print(f"⚠️  Warning: No valid proteins found for gene family {gene_name}")
                    return None
        
        except (OSError, IOError) as e:
            print(f"❌ Error: Could not create temporary file for {gene_name}: {str(e)}")
            return None
        ...
        if reference_mode == 'user':
            ...
            if not reference_file:
                print(f"⚠️  Warning: No reference file found for gene {gene_name} in {user_ref_dir}")
                print(f"   Expected files: {gene_name}.fasta, {gene_name}.fa, etc.")
                return None
            
            # Construct command for user-provided reference
            command = [
                'python', os.path.join(os.path.dirname(__file__), 'mock_wildtype_aligner.py'),
                '--gene', gene_name,
                '--input', temp_combined_fasta,
            ...
            command = [
                ...
                '--gene', gene_name,
                '--input', temp_combined_fasta,
                '--species', sepi_species,
                '--output', alignment_output_path
            ]
            
        else:
            print(f"❌ Error: Unknown reference mode '{reference_mode}' for {gene_name}")
            return None
        ...
        try:
            ...
            if os.path.isfile(alignment_output_path):
                ...
                print(f"⚠️  Warning: Expected alignment file not created for {gene_name}: {alignment_filename}")
                return None
        ...
        if temp_combined_fasta and os.path.exists(temp_combined_fasta):
            try:
                os.unlink(temp_combined_fasta)
            except OSError:
    ...
    try:
        ...
        for protein_entry in extracted_proteins:
            ...
            if not protein_fasta_path or not os.path.isfile(protein_fasta_path):
                print(f"⚠️  Warning: Protein file not found for {accession}: {protein_fasta_path}")
                continue
            ...
            try:
                ...
                print(f"⚠️  Warning: Could not read protein file {protein_fasta_path}: {str(e)}")
                continue
...
def execute_parallel_alignment(protein_manifest_path, reference_mode_info, output_dir, num_processes):
    """
    ...
    try:
        ...
        print(f"   📋 Gene families found: {', '.join(sorted(gene_groups.keys())[:5])}{'...' if len(gene_groups) > 5 else ''}")
        ...
        with multiprocessing.Pool(processes=num_processes) as pool:
            ...
            with tqdm(total=len(work_items), desc="🧬 Aligning gene families", unit="gene") as pbar:
        ...
        print(f"   ✅ Successful: {success_count}/{total_count} gene families ({success_rate:.1f}%)")
        ...
        if success_count < total_count:
            ...
            if failed_genes:
                ...
                print(f"   🔍 Failed genes: {', '.join(failed_list[:5])}{'...' if len(failed_list) > 5 else ''}")
...
def generate_alignment_manifest(input_manifest_path, successful_alignments, output_dir, reference_mode_info):
    """
    ...
    try:
        ...
        alignment_manifest['generated_by'] = 'SubScan Domino 4: The Aligner (High-Performance Parallel)'
        
        # Add timestamp
        try:
            import datetime
            alignment_manifest['generation_timestamp'] = datetime.datetime.now().isoformat()
        except ImportError:
        ...
        if reference_mode_info.get('mode') == 'user':
            alignment_manifest['alignment_metadata']['user_reference_directory'] = os.path.abspath(
                reference_mode_info.get('user_reference_dir', '')
            )
        elif reference_mode_info.get('mode') == 'sepi':
            alignment_manifest['alignment_metadata']['sepi_species'] = reference_mode_info.get('sepi_species', '')
        ...
        for gene_name, alignment_file_path in successful_alignments.items():
            ...
            if os.path.isfile(alignment_file_path):
                try:
                    alignment_entry['file_size_bytes'] = os.path.getsize(alignment_file_path)
                except OSError:
...
def main():
    ...
    if args.user_reference_dir:
        ...
        reference_mode_info = {
            'mode': 'sepi',
            'sepi_species': args.sepi_species
        }
