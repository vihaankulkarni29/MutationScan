...
def main():
    ...
    from subscan.src.subscan.reporting import ReportGenerator
    ...
    try:
        ...
        print(f"   - Total mutations: {aggregated_data['statistics']['total_mutations']}")
        print(f"   - Total co-occurrence pairs: {aggregated_data['statistics']['total_cooccurrence_pairs']}")
        print(f"   - Pipeline completion: {aggregated_data['statistics']['data_completeness']:.1f}%")
    ...
    try:
        ...
        if args.output:
            ...
            html_output_path = os.path.join(args.output_dir, f'subscan_report_{timestamp}.html')
        ...
        print(f"   - Mutations: {report_summary['data_summary']['mutations_detected']:,}")
        print(f"   - Co-occurrence pairs: {report_summary['data_summary']['cooccurrence_pairs']:,}")
        print(f"   - Gene families: {report_summary['data_summary']['gene_families']:,}")
        print(f"📈 Interactive components:")
        print(f"   - Charts: {report_summary['visualization_components']['interactive_charts']}")
        print(f"   - Data tables: {report_summary['visualization_components']['data_tables']}")
        print(f"   - MSA alignments: {report_summary['visualization_components']['msa_alignments']}")
