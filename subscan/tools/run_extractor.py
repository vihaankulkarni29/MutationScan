...
def validate_arguments(args):
    ...
    if not os.path.isfile(args.manifest):
        print(f"Error: Annotation manifest file not found: {args.manifest}", file=sys.stderr)
        sys.exit(1)
...
def extract_proteins_for_single_genome(work_item):
    ...
    try:
        ...
        if not os.path.isfile(genome_fasta_path):
            print(f"⚠️  Warning: Genome FASTA not found for {accession}: {genome_fasta_path}")
            return None
            
        if not os.path.isfile(tsv_coordinate_path):
            print(f"⚠️  Warning: TSV coordinate file not found for {accession}: {tsv_coordinate_path}")
            return None
        ...
        if os.path.isfile(mock_tool_path):
            ...
            command = [
                ...
                '--coordinates', tsv_coordinate_path,
                '--genes', gene_list_path,
                '--output', output_protein_path
            ]
        else:
            # Use real tool
            command = [
        ...
        if os.path.isfile(output_protein_path):
            ...
            print(f"⚠️  Warning: Expected output file not created for {accession}: {output_filename}")
            return None
...
def execute_parallel_extraction(annotation_manifest_path, gene_list_path, output_dir, num_processes):
    """
    ...
    try:
        ...
        with multiprocessing.Pool(processes=num_processes) as pool:
            ...
            with tqdm(total=len(work_items), desc="🧬 Extracting proteins", unit="genome") as pbar:
                
                # Submit all work items for parallel processing
                results = []
                for work_item in work_items:
                    result = pool.apply_async(extract_proteins_for_single_genome, (work_item,))
                    results.append(result)
                
                # Collect results as they complete
                for result in results:
                    try:
                        extraction_result = result.get(timeout=360)  # 6 minute timeout per genome
        ...
        print(f"   ✅ Successful: {success_count}/{total_count} genomes ({success_rate:.1f}%)")
...
def generate_protein_manifest(successful_extractions, output_dir, gene_list_path, annotation_manifest_path):
    """
    ...
    try:
        ...
        for accession, protein_fasta_path in successful_extractions:
            ...
            if os.path.isfile(protein_fasta_path):
                try:
                    protein_entry["file_size_bytes"] = os.path.getsize(protein_fasta_path)
                except OSError:
...
def main():
    ...
    try:
        ...
        if successful_extractions:
            ...
            print(f"   � Manifest ready for Domino 4: {os.path.basename(manifest_path) if manifest_path else 'Failed'}")
            return 0
        else:
