...
class MutationAnalyzer:
    ...
    def __init__(self, reference_pattern: str = "reference", min_coverage: float = 0.8):
        ...
        logger.info(
            f"MutationAnalyzer initialized with reference pattern: '{reference_pattern}'"
        )
        ...
        try:
            ...
            for seq_record in sequences:
                ...
                if coverage < self.min_coverage:
                    logger.warning(
                        f"Skipping {genome_id}: coverage {coverage:.1%} below threshold"
                    )
                    continue
            ...
            logger.error(
                f"Error analyzing mutations in {alignment_file_path}: {str(e)}"
            )
            raise
    ...
    def analyze_cooccurrence(
        self, all_mutations: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        """
        logger.info(
            f"Analyzing co-occurrence patterns for {len(all_mutations)} mutations"
        )
        ...
        logger.info(
            f"Analyzing {len(genome_mutations)} genomes across {len(gene_families)} gene families"
        )
        ...
        gene_correlations = self._calculate_gene_correlations(
            genome_mutations, gene_families
        )
        ...
        statistical_summary = {
            ...
            "avg_mutations_per_genome": (
                len(all_mutations) / len(genome_mutations) if genome_mutations else 0
            ),
            "mutation_pair_count": len(mutation_pairs),
            "significant_correlations": sum(
                1
                for corr in gene_correlations.values()
                if abs(corr.get("correlation", 0)) > 0.5
            ),
        }

        logger.info(
            f"Co-occurrence analysis complete: {statistical_summary['mutation_pair_count']} pairs analyzed"
        )
    ...
    def _identify_reference_sequences(
        ...
        for seq in sequences:
            if (
                self.reference_pattern in seq.id.lower()
                or self.reference_pattern in seq.description.lower()
            ):
    ...
    def _calculate_mutation_cooccurrence(
        self, genome_mutations: Dict[str, List[Dict]]
    ) -> Dict[str, Dict]:
    ...
    def _calculate_gene_correlations(
        self, genome_mutations: Dict[str, List[Dict]], gene_families: Set[str]
    ) -> Dict[str, Dict]:
        ...
        for genome_id, mutations in genome_mutations.items():
            ...
            gene_mutation_matrix[genome_id] = {
                gene: 1 if gene in genome_genes else 0 for gene in gene_families
            }
        ...
        for i, gene1 in enumerate(gene_list):
            for gene2 in gene_list[i + 1 :]:
                ...
                gene1_vector = [
                    gene_mutation_matrix[gid][gene1] for gid in genome_mutations.keys()
                ]
                gene2_vector = [
                    gene_mutation_matrix[gid][gene2] for gid in genome_mutations.keys()
                ]
