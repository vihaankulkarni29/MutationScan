...
def analyze_single_gene_family(gene_family_data: Dict[str, Any], 
                              analyzer: MutationAnalyzer) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        logger.info(f"Completed analysis for {gene_family}: {len(mutations)} mutations detected")
        return mutations
...
def execute_parallel_analysis(alignment_manifest: Dict[str, Any], 
                             output_dir: str, 
    ...
    if "alignment_results" in alignment_manifest:
        ...
        logger.error("Invalid manifest format: missing 'alignment_results' or 'alignments'")
        ...
        with tqdm(total=len(gene_families), desc="Analyzing gene families") as pbar:
            for future in as_completed(future_to_gene_family):
                ...
                try:
                    ...
                    logger.info(f"Gene family {gene_family}: {len(mutations)} mutations")
                    
                except Exception as e:
                    logger.error(f"Failed to analyze gene family {gene_family}: {str(e)}")
                    analysis_results.append({
                        "gene_family": gene_family,
                        "alignment_file": gene_family_data["alignment_file"],
                        "mutation_count": 0,
    ...
    logger.info(f"Parallel analysis completed: {len(all_mutations)} total mutations detected")
    ...
    if cooccurrence_results["mutation_pairs"]:
        ...
        logger.warning("No co-occurrence patterns detected - co-occurrence report not generated")
    ...
    final_results = {
        ...
        "cooccurrence_report": cooccurrence_report_path if cooccurrence_results["mutation_pairs"] else None,
        "statistics": {
            "total_gene_families": len(gene_families),
            "successful_analyses": sum(1 for r in analysis_results if r["status"] == "completed"),
            "total_mutations": len(all_mutations),
            "execution_time_seconds": execution_time,
            "mutations_per_second": len(all_mutations) / execution_time if execution_time > 0 else 0,
    ...
    logger.info(f"Processing rate: {final_results['statistics']['mutations_per_second']:.1f} mutations/second")
    
    return final_results


def generate_analysis_manifest(input_manifest: Dict[str, Any], 
...
def main():
    ...
    try:
        ...
        if "alignment_results" not in alignment_manifest and "alignments" not in alignment_manifest:
            logger.error("Invalid manifest format: missing 'alignment_results' or 'alignments'")
            return 1
        
        num_gene_families = len(alignment_manifest.get("alignment_results", alignment_manifest.get("alignments", {})))
        ...
        if analysis_results.get("cooccurrence_report"):
            print(f"  • Co-occurrence report: {analysis_results['cooccurrence_report']}")
